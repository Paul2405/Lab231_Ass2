            ...
            if (isMultiPart) {
                ...
                if (!dao.checkUserId(userId)) {
                    if (pass.equals(confirmPass)) {
                        ...
                        UserDto dto = new UserDto(userId, pass, name, role, email, phone, avatar, "active");

                        boolean create = dao.insertUser(dto);

                        if (create) {
                            ActivityDao aDao = new ActivityDao();
                            HttpSession session = request.getSession();
                            UserDto byUser = (UserDto) session.getAttribute("INFO");
                            ActivityDto activityDto = new ActivityDto(dao.getId(), "Created", byUser.getId(), byUser.getName(), byUser.getEmail());
                    ...
                    request.setAttribute("ERROR", "UserId has existed!");
                    url = "create.jsp";
                }
            }
